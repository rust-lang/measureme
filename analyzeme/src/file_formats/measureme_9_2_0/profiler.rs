...
use super::file_header::{write_file_header, FILE_MAGIC_EVENT_STREAM, FILE_MAGIC_TOP_LEVEL};
use super::raw_event::RawEvent;
use super::serialization::{PageTag, SerializationSink, SerializationSinkBuilder};
use super::stringtable::{SerializableString, StringId, StringTableBuilder};
use super::{event_id::EventId, file_header::FILE_EXTENSION};
use std::error::Error;
use std::fs;
use std::path::Path;
use std::sync::Arc;
...
impl Profiler {
    pub fn new<P: AsRef<Path>>(path_stem: P) -> Result<Profiler, Box<dyn Error + Send + Sync>> {
        Self::with_counter(
            path_stem,
            Counter::WallTime(super::counters::WallTime::new()),
        )
    }
